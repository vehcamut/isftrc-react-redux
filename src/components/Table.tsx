...
export default function CustomPaginationActionsTable() {
  ...
  const typeFilter: ISpecialistTypeQuery = {};
  // const data: ISpecialistType[] = [];
  const { data: rows, isLoading, error } = specialistAPI.useGetTypesQuery(typeFilter);
  // if (!rows) rows = [];
  // const temp = specialistAPI.useGetTypesQuery(typeFilter);
  // function createData(type: ISpecialistType) {
  //   return { name: type.name };
  // }
  // const rows = types?.map((type) => createData(type)) || [];
  // console.log(rows);
                  ...
                  <TableRow key={row.name} style={{ height: 33 }}>
                    ...
                    <TableCell style={{ width: 160 }} align="right">
                      <ButtonGroup orientation="horizontal" aria-label="vertical outlined button group">
                        <IconButton key="one" color="info">
                          <EditIcon />
                        </IconButton>
                        <IconButton key="two" color="error">
                          <DeleteIcon />
                        </IconButton>
                      </ButtonGroup>
                    </TableCell>
